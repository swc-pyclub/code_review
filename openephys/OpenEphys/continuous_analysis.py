# -*- coding: utf-8 -*-
"""Functions related to analysis of continuous data
...
def do_sta(analog_signal, event, window=[-.5, .5], verbose=True,
    ...
    for e in times:
        if not event_in_samples:
            if s_rate is None:
                raise IOError('Event is not in samples and I do not know the sampling rate')
    ...
    for ind, e in enumerate(times):
        ...
        if ind_ev + winind[1] > len(analog_signal):
            ...
            if verbose:
                print('skip first cause it does not fit')
            continue
        out[ind] = np.asarray(analog_signal[ind_ev + winind[0]: ind_ev + winind[1]], dtype=dtype)
    return out
